...
function updatePlayingSelection() { const count = playingSelectedAccounts().length; if ($('#playingSelectionCount')) $('#playingSelectionCount').textContent = String(count); }
function renderPlayingAccounts() { const root = $('#playingAccounts'); if (!root) return; root.innerHTML = state.clients.length ? state.clients.map((client) => `<div class="playing-account"><label><input type="checkbox" value="${escapeHTML(client.name)}" /><span class="target-avatar">${client.avatar ? `<img src="${escapeHTML(client.avatar)}" alt="" />` : escapeHTML((client.nickname || client.name || '?')[0])}</span><span><strong>${escapeHTML(client.nickname || client.name)}</strong><small>${escapeHTML(client.username || client.name)}</small></span></label><button class="playing-customize" type="button">Customize</button><textarea class="playing-account-phrases" data-account="${escapeHTML(client.name)}" rows="2" placeholder="كلمات هذا الحساب (سطر لكل زر، اختياري)"></textarea></div>`).join('') : '<div class="task-empty">لا توجد حسابات متصلة</div>'; root.querySelectorAll('input[type="checkbox"]').forEach((input) => input.addEventListener('change', updatePlayingSelection)); root.querySelectorAll('.playing-customize').forEach((button) => button.addEventListener('click', () => { const area = button.parentElement.querySelector('.playing-account-phrases'); area.classList.toggle('is-visible'); button.textContent = area.classList.contains('is-visible') ? 'Hide' : 'Customize'; })); updatePlayingSelection(); }
function addPlayingStep(step = {}) { const root = $('#playingSteps'); const row = document.createElement('div'); row.className = 'playing-step'; row.innerHTML = `<span class="playing-step-number">${root.children.length + 1}</span><input class="playing-button-name" type="text" maxlength="80" placeholder="اسم الزر، مثال: دخول" value="${escapeHTML(step.button || '')}" /><input class="playing-phrase" type="text" maxlength="500" placeholder="كلمة أو بدائل بـ | (اختياري)" value="${escapeHTML(step.phrase || '')}" /><button class="playing-remove" type="button">×</button>`; row.querySelector('.playing-remove').addEventListener('click', () => { row.remove(); [...root.children].forEach((item, index) => { item.querySelector('.playing-step-number').textContent = index + 1; }); }); root.appendChild(row); }
...
function renderPlayingSessions() { const sessions = state.playingSessions || []; const active = sessions.filter((session) => session.active); $('#playingStatus').textContent = `${active.length} active`; $('#navPlayingCount').textContent = String(active.length); $('#playingSessions').innerHTML = sessions.length ? sessions.map((session) => `<div class="playing-session ${session.active ? 'is-active' : ''}"><div><strong>${escapeHTML(session.account)}</strong><small>${escapeHTML(session.scenarioName || 'Playing')} · ${escapeHTML(session.status || (session.active ? 'running' : 'saved'))} · ${session.steps?.length || 0} actions</small>${session.lastResult?.error ? `<small class="playing-last">${escapeHTML(session.lastResult.error)}</small>` : session.lastAction ? `<small class="playing-last">آخر كلمة: ${escapeHTML(session.lastAction.phrase || 'بدون رسالة')}</small>` : ''}</div><div class="playing-session-actions"><button class="ghost-button playing-stop-one" data-account="${escapeHTML(session.account)}" type="button">${session.active ? 'Stop' : 'Stopped'}</button><button class="ghost-button playing-remove-one" data-account="${escapeHTML(session.account)}" type="button">Remove</button></div></div>`).join('') : '<div class="task-empty">لا توجد جلسات محفوظة</div>'; document.querySelectorAll('.playing-stop-one').forEach((button) => button.addEventListener('click', async () => { if (button.textContent === 'Stopped') return; try { await post('/api/playing/stop', { accounts: [button.dataset.account] }); await loadPlayingSessions(); } catch (error) { toast(error.message, 'error'); } })); document.querySelectorAll('.playing-remove-one').forEach((button) => button.addEventListener('click', async () => { if (!window.confirm(`إزالة ${button.dataset.account} من السيناريو؟`)) return; try { await post('/api/playing/delete', { account: button.dataset.account }); await loadPlayingSessions(); } catch (error) { toast(error.message, 'error'); } })); }
async function loadPlayingSessions() { try { const data = await api('/api/playing/sessions'); state.playingSessions = data.sessions || []; renderPlayingSessions(); } catch (error) { console.warn('[playing]', error); } }
async function loadPlayingChannels(filter = '') { try { const guildSelect = $('#playingGuildSelect'); const roomSelect = $('#playingChannelSelect'); if (!guildSelect || !roomSelect) return; const data = await api(`/api/playing/channels${guildSelect.value ? `?guildId=${encodeURIComponent(guildSelect.value)}` : ''}`); if (!guildSelect.value) { guildSelect.innerHTML = data.guilds?.length ? `<option value="">Select a server</option>${data.guilds.map((guild) => `<option value="${escapeHTML(guild.id)}">${escapeHTML(guild.name)}</option>`).join('')}` : '<option value="">No servers available</option>'; roomSelect.innerHTML = '<option value="">Select a server first</option>'; roomSelect.disabled = true; $('#playingChannelSearch').disabled = true; return; } const query = String(filter || '').trim().toLowerCase(); const channels = (data.channels || []).filter((channel) => !query || channel.name.toLowerCase().includes(query)); roomSelect.innerHTML = channels.length ? `<option value="">Select a game room</option>${channels.map((channel) => `<option value="${escapeHTML(channel.id)}" data-channel-name="${escapeHTML(channel.name)}">#${escapeHTML(channel.name)} · ${channel.accounts.length} account${channel.accounts.length === 1 ? '' : 's'}</option>`).join('')}` : '<option value="">No matching text rooms</option>'; roomSelect.disabled = false; $('#playingChannelSearch').disabled = false; roomSelect.value = $('#playingChannelId').value || ''; } catch (error) { console.warn('[playing channels]', error); } }
async function savePlaying() { const accounts = playingSelectedAccounts(); const baseSteps = playingSteps(); if (!accounts.length) return toast('اختر حسابًا واحدًا على الأقل', 'error'); if (!baseSteps.length) return toast('أضف اسم زر واحدًا على الأقل', 'error'); const selectedChannel = $('#playingChannelSelect')?.selectedOptions[0]; const body = { scenarioName: $('#playingScenarioName')?.value.trim() || 'Playing scenario', channelId: $('#playingChannelSelect')?.value || $('#playingChannelId').value.trim(), channelName: selectedChannel?.dataset.channelName || $('#playingChannelName').value.trim(), intervalMs: Number($('#playingInterval').value || 5) * 1000 }; if (!body.channelId) return toast('اختر قناة اللعبة أولًا', 'error'); try { for (const account of accounts) { const customText = document.querySelector(`.playing-account-phrases[data-account="${CSS.escape(account)}"]`)?.value || ''; const custom = customText.trim() ? customText.split('\n') : null; const steps = baseSteps.map((step, index) => ({ button: step.button, phrase: custom && custom[index] !== undefined ? custom[index].trim() : step.phrase })); await api('/api/playing/save', { method: 'POST', body: JSON.stringify({ ...body, account, steps }) }); } feedback('#playingFeedback', `تم حفظ الإعداد لـ ${accounts.length} حساب`, 'success'); await loadPlayingSessions(); } catch (error) { feedback('#playingFeedback', error.message, 'error'); } }
async function startPlaying() { const accounts = playingSelectedAccounts(); if (!accounts.length) return toast('اختر الحسابات التي تريد تشغيلها', 'error'); const accountDelayMs = Math.max(0, Number($('#playingAccountDelay')?.value || 0) * 1000); try { await api('/api/playing/start', { method: 'POST', body: JSON.stringify({ accounts, accountDelayMs }) }); toast('تم تشغيل جلسات Playing بالتدرج المحدد', 'success'); await loadPlayingSessions(); } catch (error) { toast(error.message, 'error'); } }
...
async function loadPlayingEvents() { try { const data = await api('/api/playing/events'); $('#playingEvents').innerHTML = data.events?.length ? data.events.map((event) => `<div class="playing-event"><span>${escapeHTML(event.event)}</span><strong>${escapeHTML(event.account || 'workspace')}</strong><small>${escapeHTML(event.result?.button || event.reason || '')} · ${escapeHTML(new Date(event.time).toLocaleTimeString())}</small></div>`).join('') : '<div class="task-empty">لا يوجد نشاط بعد</div>'; } catch (error) { console.warn('[playing events]', error); } }
async function previewPlaying() { const steps = playingSteps(); try { const data = await api('/api/playing/preview', { method: 'POST', body: JSON.stringify({ steps }) }); $('#playingPreview').innerHTML = data.preview.map((step) => `<div class="playing-preview-row"><b>${step.order}</b><strong>${escapeHTML(step.button)}</strong><span>${escapeHTML(step.phrase || 'بدون رسالة')}</span></div>`).join(''); } catch (error) { toast(error.message, 'error'); } }
function initPlaying() { addPlayingStep(); renderPlayingAccounts(); loadPlayingChannels(); $('#playingGuildSelect')?.addEventListener('change', () => { $('#playingChannelId').value = ''; $('#playingChannelName').value = ''; $('#playingChannelSearch').value = ''; loadPlayingChannels(); }); $('#playingChannelSearch')?.addEventListener('input', (event) => loadPlayingChannels(event.target.value)); $('#playingChannelSelect')?.addEventListener('change', (event) => { $('#playingChannelId').value = event.target.value; $('#playingChannelName').value = event.target.selectedOptions[0]?.dataset.channelName || ''; }); document.querySelectorAll('[data-playing-tab]').forEach((button) => button.addEventListener('click', () => { document.querySelectorAll('[data-playing-tab]').forEach((item) => item.classList.toggle('is-active', item === button)); document.querySelectorAll('[data-playing-pane]').forEach((pane) => { pane.classList.toggle('is-active', pane.dataset.playingPane === button.dataset.playingTab); }); if (button.dataset.playingTab === 'activity') loadPlayingEvents(); })); $('#playingAddStep')?.addEventListener('click', () => addPlayingStep()); $('#playingSelectAll')?.addEventListener('click', () => { const boxes = [...document.querySelectorAll('#playingAccounts input')]; const all = boxes.length && boxes.every((box) => box.checked); boxes.forEach((box) => { box.checked = !all; }); updatePlayingSelection(); }); $('#playingSave')?.addEventListener('click', savePlaying); $('#playingPreviewButton')?.addEventListener('click', previewPlaying); $('#playingStart')?.addEventListener('click', startPlaying); $('#playingStop')?.addEventListener('click', stopPlaying); $('#playingEmergency')?.addEventListener('click', async () => { if (!window.confirm('إيقاف جميع جلسات Playing؟')) return; await api('/api/playing/emergency-stop', { method: 'POST', body: '{}' }); toast('تم الإيقاف الطارئ', 'success'); await loadPlayingSessions(); }); $('#playingRefreshEvents')?.addEventListener('click', loadPlayingEvents); loadPlayingSessions(); }
...
function initActivity() { $('#activityExpandButton')?.addEventListener('click', () => { const filter = $('#activityAccountFilter'); filter.innerHTML = '<option value="">All accounts</option>' + state.clients.map((client) => `<option value="${escapeHTML(client.name)}">${escapeHTML(client.nickname || client.name)}</option>`).join(''); $('#activityModal').hidden = false; renderFullActivity(0); }); $('#activityAccountFilter')?.addEventListener('change', () => renderFullActivity(0)); $('#activityClearButton')?.addEventListener('click', clearActivity); $('#activityClearModalButton')?.addEventListener('click', clearActivity); const closeActivity = () => { $('#activityModal').hidden = true; }; $('#activityCloseButton')?.addEventListener('click', closeActivity); $('#activityCloseTopButton')?.addEventListener('click', closeActivity); $('#activityModal')?.addEventListener('click', (event) => { if (event.target.id === 'activityModal') closeActivity(); }); $('#activityPrevButton')?.addEventListener('click', () => renderFullActivity(state.activityPage - 1)); $('#activityNextButton')?.addEventListener('click', () => renderFullActivity(state.activityPage + 1)); }
