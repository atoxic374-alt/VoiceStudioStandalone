...
async function loadClients(preferred = '') {
  ...
  select.innerHTML = state.clients.length
    ? state.clients.map((client) => `<option value="${escapeHTML(client.name)}">${escapeHTML(client.name)}</option>`).join('')
...
async function bulkConnect() {
  ...
  const accounts = lines.map((line) => {
    ...
    if (separator > 0) return { name: line.slice(0, separator).trim(), token: line.slice(separator + 1).trim() };
    return { token: line };
...
function openDisconnect() {
  ...
  list.innerHTML = state.clients.map((client) => `<label class="account-target disconnect-target"><input type="checkbox" value="${escapeHTML(client.name)}" checked /><span class="target-avatar">${client.avatar ? `<img src="${escapeHTML(client.avatar)}" alt="" />` : escapeHTML((client.nickname || client.name || '?')[0])}</span><span class="target-copy"><strong>${escapeHTML(client.name)}</strong><small>${escapeHTML(client.nickname || client.displayName || client.username || client.name)} · ID: ${escapeHTML(client.id || '—')}</small></span><span class="target-status">Connected</span></label>`).join('');
...
function openTaskDetails(taskId) {
  ...
  $('#taskDetailsAccounts').innerHTML = (task.accounts || []).map((name) => {
    ...
    return `<div class="task-detail-account"><div class="task-detail-account-main"><span class="task-detail-avatar">${escapeHTML((name || '?')[0])}</span><div><strong>${escapeHTML(name)}</strong><small>${escapeHTML(client?.nickname || client?.displayName || client?.username || name)}</small></div></div><div class="task-detail-account-state"><span>${escapeHTML(detail)}</span><small class="${result?.ok === false ? 'is-error' : 'is-ok'}">${result?.ok === false ? escapeHTML(result.error || 'فشل آخر تنفيذ') : 'آخر تنفيذ ناجح أو قيد المتابعة'}</small></div></div>`;
