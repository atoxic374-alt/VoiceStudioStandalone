...
function renderPlayingSessions() { const sessions = state.playingSessions || []; const active = sessions.filter((session) => session.active); $('#playingStatus').textContent = `${active.length} active`; $('#navPlayingCount').textContent = String(active.length); $('#playingSessions').innerHTML = sessions.length ? sessions.map((session) => `<div class="playing-session ${session.active ? 'is-active' : ''}"><div><strong>${escapeHTML(session.account)}</strong><small>${escapeHTML(session.scenarioName || 'Playing')} · ${escapeHTML(session.status || (session.active ? 'running' : 'saved'))} · ${session.steps?.length || 0} actions</small>${session.lastResult?.error ? `<small class="playing-last">${escapeHTML(session.lastResult.error)}</small>` : session.lastAction ? `<small class="playing-last">آخر كلمة: ${escapeHTML(session.lastAction.phrase || 'بدون رسالة')}</small>` : ''}</div><button class="ghost-button playing-stop-one" data-account="${escapeHTML(session.account)}" type="button">${session.active ? 'Stop' : 'Delete'}</button></div>`).join('') : '<div class="task-empty">لا توجد جلسات محفوظة</div>'; document.querySelectorAll('.playing-stop-one').forEach((button) => button.addEventListener('click', async () => { try { await post(button.textContent === 'Stop' ? '/api/playing/stop' : '/api/playing/delete', { accounts: [button.dataset.account], account: button.dataset.account }); await loadPlayingSessions(); } catch (error) { toast(error.message, 'error'); } })); }
