...
async function loadPlayingChannels(filter = '') { try { const data = await api('/api/playing/channels'); const select = $('#playingChannelSelect'); if (!select) return; const query = String(filter || '').trim().toLowerCase(); const channels = (data.channels || []).filter((channel) => !query || `${channel.guildName} ${channel.name}`.toLowerCase().includes(query)); select.innerHTML = channels.length ? `<option value="">Select a game channel</option>${channels.map((channel) => `<option value="${escapeHTML(channel.id)}" data-channel-name="${escapeHTML(channel.name)}">${escapeHTML(channel.guildName)} / #${escapeHTML(channel.name)} · ${channel.accounts.length} account${channel.accounts.length === 1 ? '' : 's'}</option>`).join('')}` : '<option value="">No matching text channels</option>'; select.value = $('#playingChannelId').value || ''; } catch (error) { console.warn('[playing channels]', error); } }
