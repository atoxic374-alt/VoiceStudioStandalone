...
async function savePlaying() { const accounts = playingSelectedAccounts(); const baseSteps = playingSteps(); if (!accounts.length) return toast('اختر حسابًا واحدًا على الأقل', 'error'); if (!baseSteps.length) return toast('أضف اسم زر واحدًا على الأقل', 'error'); const body = { scenarioName: $('#playingScenarioName')?.value.trim() || 'Playing scenario', channelId: $('#playingChannelId').value.trim(), channelName: $('#playingChannelName').value.trim(), intervalMs: Number($('#playingInterval').value || 5) * 1000 }; if (!body.channelId) return toast('أدخل ID القناة النصية', 'error'); try { for (const account of accounts) { const customText = document.querySelector(`.playing-account-phrases[data-account="${CSS.escape(account)}"]`)?.value || ''; const custom = customText.trim() ? customText.split('\n') : null; const steps = baseSteps.map((step, index) => ({ button: step.button, phrase: custom && custom[index] !== undefined ? custom[index].trim() : step.phrase })); await api('/api/playing/save', { method: 'POST', body: JSON.stringify({ ...body, account, steps }) }); } feedback('#playingFeedback', `تم حفظ الإعداد لـ ${accounts.length} حساب`, 'success'); await loadPlayingSessions(); } catch (error) { feedback('#playingFeedback', error.message, 'error'); } }
