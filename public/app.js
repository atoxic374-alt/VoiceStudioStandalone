...
function renderPlayingEvents() { const events = state.playingEvents || []; const query = ($('#playingEventSearch')?.value || '').trim().toLowerCase(); const kind = $('#playingEventFilter')?.value || ''; const visible = events.filter((event) => (!kind || event.event === kind) && (!query || JSON.stringify(event).toLowerCase().includes(query))); const labels = { 'button.waiting': 'Waiting for button', 'button.found': 'Button found', 'button.click.started': 'Click started', 'button.click.completed': 'Click completed', 'button.click.failed': 'Click failed', 'phrase.send.started': 'Message sending', 'phrase.send.completed': 'Message sent', 'phrase.send.failed': 'Message failed', 'action.completed': 'Action completed', 'action.failed': 'Action failed' }; $('#playingEvents').innerHTML = visible.length ? visible.map((event) => { const detail = event.result?.error || event.error || event.reason || event.label || (event.result?.button ? `Button: ${event.result.button}` : event.requested ? `Requested: ${event.requested}` : event.phrase ? `Message: ${event.phrase}` : ''); const found = event.available?.length || event.result?.available?.length ? `Available: ${(event.available || event.result.available).join(', ')}` : ''; return `<div class="playing-event"><span class="playing-event-kind">${escapeHTML(labels[event.event] || event.event)}</span><strong>${escapeHTML(event.account || 'workspace')}</strong><small>${escapeHTML(detail)}${found ? ` · ${escapeHTML(found)}` : ''}</small><time>${escapeHTML(new Date(event.time).toLocaleTimeString())}</time></div>`; }).join('') : '<div class="task-empty">لا توجد نتائج مطابقة</div>'; }
...
function initPlaying() { addPlayingStep(); restorePlayingDraft(); renderPlayingAccounts(); loadPlayingChannels(); const refreshChecklist = () => { renderPlayingChecklist(); savePlayingDraft(); }; $('#playingGuildSelect')?.addEventListener('change', () => { $('#playingChannelId').value = ''; $('#playingChannelName').value = ''; $('#playingChannelSearch').value = ''; state.playingChannelAccounts = []; loadPlayingChannels(); refreshChecklist(); }); $('#playingChannelSearch')?.addEventListener('input', (event) => loadPlayingChannels(event.target.value)); $('#playingChannelSelect')?.addEventListener('change', (event) => { $('#playingChannelId').value = event.target.value; $('#playingChannelName').value = event.target.selectedOptions[0]?.dataset.channelName || ''; state.playingChannelAccounts = state.playingChannels?.find((channel) => channel.id === event.target.value)?.accounts || []; renderPlayingAccounts(); refreshChecklist(); }); document.querySelectorAll('#playingScenarioName, #playingInterval, #playingAccountDelay').forEach((input) => input.addEventListener('input', refreshChecklist)); $('#playingEventSearch')?.addEventListener('input', renderPlayingEvents); $('#playingEventFilter')?.addEventListener('change', renderPlayingEvents); document.querySelectorAll('[data-playing-tab]').forEach((button) => button.addEventListener('click', () => { document.querySelectorAll('[data-playing-tab]').forEach((item) => item.classList.toggle('is-active', item === button)); document.querySelectorAll('[data-playing-pane]').forEach((pane) => { pane.classList.toggle('is-active', pane.dataset.playingPane === button.dataset.playingTab); }); if (button.dataset.playingTab === 'activity') loadPlayingEvents(); })); $('#playingAddStep')?.addEventListener('click', () => addPlayingStep()); $('#playingSelectAll')?.addEventListener('click', () => { const boxes = [...document.querySelectorAll('#playingAccounts input:not(:disabled)')]; const all = boxes.length && boxes.every((box) => box.checked); boxes.forEach((box) => { box.checked = !all; }); updatePlayingSelection(); }); $('#playingSave')?.addEventListener('click', savePlaying); $('#playingSaveRun')?.addEventListener('click', saveAndRunPlaying); $('#playingStickySave')?.addEventListener('click', savePlaying); $('#playingStickyCancel')?.addEventListener('click', cancelPlayingEdit); $('#playingCancelEdit')?.addEventListener('click', cancelPlayingEdit); $('#playingStickyRun')?.addEventListener('click', saveAndRunPlaying); $('#playingPreviewButton')?.addEventListener('click', previewPlaying); $('#playingStart')?.addEventListener('click', startPlaying); $('#playingStop')?.addEventListener('click', stopPlaying); $('#playingEmergency')?.addEventListener('click', async () => { const active = (state.playingSessions || []).filter((session) => session.active).length; if (!window.confirm(`إيقاف ${active} جلسة Playing؟`)) return; await api('/api/playing/emergency-stop', { method: 'POST', body: '{}' }); toast('تم الإيقاف الطارئ', 'success'); await loadPlayingSessions(); }); $('#playingRefreshEvents')?.addEventListener('click', loadPlayingEvents); loadPlayingSessions(); renderPlayingChecklist(); }
