...
Open [http://localhost:5050](http://localhost:5050). Connected Discord tokens are stored encrypted in `data/accounts.enc` and restored automatically after restart; voice sessions are persisted in `data/voice-sessions.json`. The server also accepts manually uploaded compatibility names such as `accounts.enc.txt`, `voice-sessions.json.txt`, `automation-tasks.json.txt`, and `playing-sessions.json.txt`, then migrates the account file to its canonical name. Automatic account restore supports up to 500 saved accounts by default; accounts skipped because of a temporary connection failure or a lower `MAX_RESTORED_ACCOUNTS` setting remain in encrypted storage and are not overwritten by the accounts that did reconnect. Set `MAX_RESTORED_ACCOUNTS` lower on small instances if needed. Set `DATA_ENCRYPTION_KEY` to a stable random value in production; the loader also tries the current `APP_PASSWORD` and the local-development fallback so an older export can be migrated when its original key is still available. Set `DATA_DIR` only when the Railway Volume is mounted at a custom path. Use HTTPS or `localhost` when deploying the browser UI so browser media permissions are available.
...
For permanent Railway data, create and mount a Railway Volume at `/app/data` (or set `DATA_DIR` to the mounted path). The application writes account exports with an atomic temporary-file rename, preserves saved records across partial reconnects and invalid-token events, and performs a final synchronous persistence pass on `SIGTERM` and `SIGINT`. After authentication, `/api/storage/status` reports the active data directory, account file, file sizes, saved-account count, and connected-account count so the deployment can be verified without inspecting tokens.
